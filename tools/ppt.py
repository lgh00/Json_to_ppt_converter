...
class PptTool(Tool):
    def _invoke(self, tool_parameters: dict[str, Any]) -> Generator[ToolInvokeMessage]:
        ...
        json_content = tool_parameters.get("json_content", "")
        ...
        try:
            ...
            pptx_bytes = self._convert_json_to_pptx(json_content, title, theme)
    ...
    def _convert_json_to_pptx(self, json_content: str, title: str = "default_ppt_name", theme: str = "default") -> bytes:
        """
        将JSON字符串转换为PowerPoint文件
        """
        # json:标题页，居中对齐摆放
        # json:内容页，上方为子标题，格式为"节标题：内容标题"，下方为内容
        # 1.解析JSON字符串
        ...
        page_w = ppt.slide_width
        page_h = ppt.slide_height
        print(f"page_w{page_w/Inches(1)}, page_h{page_h/Inches(1)}")
        print(f"page_w{page_w/Cm(1)}, page_h{page_h/Cm(1)}")
        title_box_w = Inches(8)    # 文本框宽度
        title_box_h = Inches(2)    # 文本框高度
        left = (page_w - title_box_w) / 2
        top  = (page_h - title_box_h) / 2
        title_shape = title_slide.shapes.add_textbox(left, top, title_box_w, title_box_h)
        ...
        # 4.添加内容页
        for singal_section in content:
            section_title = singal_section["section_title"]
            section_content = singal_section["section_content"]
            for page in section_content:
                page_summary_title = page["page_summary_title"]
                page_content = page["page_content"]
                page_title = section_title + "-" + page_summary_title
                content_slide_layout = ppt.slide_layouts[6]
                page_slide = ppt.slides.add_slide(content_slide_layout)
                ## 4.1 页标题
                page_title_box_w = Inches(9)    # 文本框宽度
                page_title_box_h = Inches(0.7)
                left = (page_w - page_title_box_w) / 2
                top  = Inches(0.5)
                page_title_shape = page_slide.shapes.add_textbox(left, top, page_title_box_w, page_title_box_h)
                tf = page_title_shape.text_frame
                p = tf.paragraphs[0]
                p.text = page_title
                font = p.font
                font.name = "微软雅黑"
                ...
                page_content_box_w = Inches(9)
                page_content_box_h = Inches(5.5)
                left = (page_w - page_content_box_w) / 2
                top = Inches(1.5)
                page_content_shape = page_slide.shapes.add_textbox(left, top, page_content_box_w, page_content_box_h)
                tf = page_content_shape.text_frame
                tf.word_wrap = True
